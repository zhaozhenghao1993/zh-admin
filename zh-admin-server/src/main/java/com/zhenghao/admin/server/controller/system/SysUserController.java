...
import com.zhenghao.admin.server.entity.dto.SysUserPasswordDTO;
...
public class SysUserController extends AbstractController {
    ...
    public Result updatePswdByUser(@RequestBody @Valid SysUserPasswordDTO password, HttpServletRequest request, BindingResult results) {
        if (results.hasErrors()) {
            return Result.ofFail(results.getFieldError().getDefaultMessage());
        }
        RSAUtils.RsaKey rsaKey = SessionUtils.getRsaKeyFromSession(request);
        if (rsaKey == null) {
            return Result.ofFail("修改超时，请重新修改!");
        }
        try {
            password.setPassword(RSAUtils.decrypt(password.getPassword(), rsaKey.getPrivateKey()));
            password.setOldPassword(RSAUtils.decrypt(password.getOldPassword(), rsaKey.getPrivateKey()));
        } catch (Exception e) {
    ...
    public Result<Page<SysUserEntity>> list(@RequestParam Map<String, Object> params) {
        if (getUserId() != SystemConstants.SUPER_ADMIN) {
    ...
    @SysLog("新增用户")
    @PostMapping("")
    public Result save(@RequestBody @Valid SysUserEntity user, HttpServletRequest request, BindingResult results) {
        if (results.hasErrors()) {
            return Result.ofFail(results.getFieldError().getDefaultMessage());
        }
        RSAUtils.RsaKey rsaKey = SessionUtils.getRsaKeyFromSession(request);
        ...
        try {
            user.setPassword(RSAUtils.decrypt(user.getPassword(), rsaKey.getPrivateKey()));
        } catch (Exception e) {
            logger.error("rsa decrypt exception", e);
            return Result.ofFail("修改超时，请重新修改!");
        }
        user.setAvatar(UploadConstants.USER_AVATAR_DEFAULT_PATH); // 给个默认头像
    ...
    @SysLog("重置密码")
    @PutMapping("/{id}/reset")
    public Result updatePswd(@PathVariable("id") Long id, @RequestBody SysUserEntity user, HttpServletRequest request) {
        RSAUtils.RsaKey rsaKey = SessionUtils.getRsaKeyFromSession(request);
        if (rsaKey == null) {
            return Result.ofFail("修改超时，请重新修改!");
        }
